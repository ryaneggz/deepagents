...
# https://github.com/cloudflare/mcp-server-cloudflare/tree/main/apps/docs-vectorize#prompt-examples
...
# Example Query 
query = (
    "Do Cloudflare Workers costs depend on response sizes? "
    "I want to serve some images (map tiles) from an R2 bucket and I'm concerned about costs."
...
async def main():
    ...
    async for chunk in agent.astream({"messages": [{"role": "user", "content": query}]}):
        if chunk.get('agent'):
            ...
            if chunk['agent']['messages'][-1].tool_calls:
                print("\n*** TOOL INPUT ***")
                print(chunk['agent']['messages'][-1].tool_calls)
        if chunk.get('tools'):
            print("\n*** TOOL OUTPUT ***")
